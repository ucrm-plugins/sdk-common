...
// =================================================================================================================
     ...
     * @return array Data which can be serialized by <b>json_encode</b>, as a value of any type other than a resource.
     * @since 5.4.0
     */
    ...
    public function toJSON(string $method = "", bool $filter = true, int $options = 0): string
    {
        ...
        // Set up the Reflection instance for the calling class.
        ...
        foreach($properties as $property)
        {
            ...
            // Create an AnnotationReader for this property and get all of its annotations.
        ...
        // Now replace any occurrences of the NULL delimiter with the actual 'null' value and return the JSON string.
        return str_replace("\"".self::NULL_DELIMITER."\"", "null", $json);
    }
    
    /**
     * Converts the RestObject to its associative array representation, removing and values that should not be provided
    ...
    {
        $json = $this->toJSON($method, $filter);
    ...
    /**
     * Checks to determine the validity of a RestObject, by comparing each property's value to NULL, when it has been
    ...
    public function validate(string $method, array &$missing = null, array &$ignored = null): bool
    {
        ...
        foreach($properties as $property)
        {
            ...
            if(array_key_exists(ucfirst($method)."Required", $params) || array_key_exists($method."-required", $params))
            {
                ...
                else if($conditional !== "" && Strings::contains($conditional, "`"))
                {
