...
use SpaethTech\UCRM\SDK\Collections\Exceptions\CollectionComparisonException;
use SpaethTech\UCRM\SDK\Collections\Exceptions\CollectionIndexException;
use SpaethTech\UCRM\SDK\Collections\Exceptions\CollectionTypeException;
...
class Collection implements JsonSerializable, Countable, Iterator
{
     ...
     * @return Collectible[] Data to be serialized by <b>json_encode</b>, which can be of any type except a resource.
     */
    ...
    {
        if(!$range)
            throw new CollectionIndexException("The range: '".
    ...
    public function slice(int $index, int $count): Collection
    {
        ...
        return new Collection($this->type, array_slice($this->elements ?? [], $index, $count));
    }
    ...
    public function merge(Collection $collection): Collection
    {
        if($this->type() !== $collection->type())
            throw new CollectionTypeException("The merging Collection type '{$collection->type()}' must match the original ".
    ...
    {
        if($count > $this->count())
            throw new CollectionIndexException("The count '$count' is more than the number of ".
    ...
    {
        return $this->elements && ($index < count($this->elements));
    }
    ...
    public function validIndex(int $index): int
    {
        if(!$this->hasIndex($index))
            throw new CollectionIndexException("The index: '$index' is invalid!");
